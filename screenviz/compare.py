...
class CompareScreens:
    def __init__(
        ...
        self.df_a = self.load_dataframe(filename_a, merge_column_a, variable_column_a, threshold_column_a, "a")
        self.df_b = self.load_dataframe(filename_b, merge_column_b, variable_column_b, threshold_column_b, "b")
        self.df = pd.merge(self.df_a, self.df_b, left_on=f"{merge_column_a}_a", right_on=f"{merge_column_b}_b")
        self.df = self.classify_dataframe(self.df)

    def load_dataframe(self, filename: str, merge_column:str, variable_column: str, threshold_column: str, suffix: str) -> pd.DataFrame:
        dataframe = pd.read_csv(filename, sep="\t")
        assert merge_column in dataframe.columns, f"Column {merge_column} not found in {filename}"
        assert variable_column in dataframe.columns, f"Column {variable_column} not found in {filename}"
        assert threshold_column in dataframe.columns, f"Column {threshold_column} not found in {filename}"
        dataframe = dataframe\
            .loc[:, [merge_column, variable_column, threshold_column]]\
            .rename(columns={
    ...
    def classify_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["sizing"] = df.apply(lambda x: 10 if x.is_significant_a or x.is_significant_b else 5, axis=1)
        return df
    ...
    def plot_volcano(self, output: str = "comparison.html"):
        ...
        if self.log_transform_a:
            self.df[f"log_{self.variable_column_a}_a"] = -np.log10(self.df[f"{self.variable_column_a}_a"])
            variable_name_x = f"log_{self.variable_column_a}_a"

        if self.log_transform_b:
            self.df[f"log_{self.variable_column_b}_b"] = -np.log10(self.df[f"{self.variable_column_b}_b"])
